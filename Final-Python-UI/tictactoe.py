...
class Game:
    def __init__(self, gameLogic, dim=3):
        self.dim = dim
    ...
    def move(self,pos):
        ...
        #Get UI coordinates
        tileX = pos[0] // self.dim
        tileY = pos[1] // self.dim
        spaceX = pos[0] % self.dim
        spaceY = pos[1] % self.dim


        if self.currPlayer == -1:
            self.gameLogic.makeMove(TileStates.PLAYER_O, tileX, tileY, spaceX, spaceY)
            #gameUI.setMarker(TileStates.PLAYER_O, spaceX, spaceY)
        else:
            self.gameLogic.makeMove(TileStates.PLAYER_X, tileX, tileY, spaceX, spaceY)
            #gameUI.setMarker(TileStates.PLAYER_X, spaceX, spaceY)
